...
prediction reduces urgent SLO miss rate by about $13\%$ under same-model
co-location, in \emph{both} co-run states, by cutting expensive serial
fallbacks by 73--81\%. It does not detect the bistable state, which is
...
\midrule
deadline actions & $+1.05\%$ & $+3.02\%$ \\
\textbf{probe} & $\mathbf{-13.14\%}$ & $\mathbf{-14.20\%}$ \\
serial fallbacks & $-73\%$ & $-81\%$ \\
video goodput & $+0.04\%$ & $-0.01\%$ \\
\bottomrule
\end{tabular}
\caption{Probe intervals \ci{-0.0530}{-0.0127} and
\ci{-0.0604}{-0.0106}; both deadline intervals span zero.}
\end{table}
