...
| **The mask was honoured, checked 2026-09-03** | The cross-architecture measurement was taken at 52+52 through `hipExtStreamCreateWithCUMask`, and that path honours 52 exactly in both halves on gfx90a, disjointly. It matters because the process path does **not**: `ROC_GLOBAL_CU_MASK` is exact only at or below 32 and at 64 and 104, and silently rounds 52 to 64, 78 to 104, 96 to 104. An unhonoured mask would have produced an unusually low and tight penalty -- which is the shape 1.2336, sd 0.0030 has -- so this was worth checking rather than assuming. `experiments/probes/gfx90a/mask_contract_20260903.json`. |
...
| | |
| --- | --- |
| **Claim** | The same-model co-run penalty is **not** a function of how much of the die is busy. A slice with `N-1` peers pays far more than the same slice with **one** peer filling the same units. gfx1201, all four widths: **1.050 / 1.367 / 2.318 / 4.520** at 1 / 2 / 4 / 8 ways. Against the pairwise entry for a slice of the same width: **+10.5%** at two ways, **+77.3%** at four, **+237.8%** at eight. |
| **Evidence** | `scripts/run_amd_nway_steps.py`, N resident step adapters on N disjoint masks, six episodes, the last reported, **timed by the wall clock with the device synchronised first**. `experiments/probes/gfx1201/nway_walled/`. Every cell's pre-episode solo lands on that width's measured quota curve, 3-6% fast and by the same margin at every width: 32u 108.5 against 115.5, 16u 153.4 against 157.5, 8u 262.8 against 269, 4u 502.2 against 521. Per-slice spread at eight ways is 4.418-4.641. gfx90a, same instrument, all four widths: **1.024 / 1.247 / 1.507 / 2.245** at 1 / 2 / 4 / 8 ways, `experiments/probes/gfx90a/walled_final/`, excess **+2.4% / +18.0% / +65.6%**. |
| **Both architectures, and the confound in comparing them** | The shape is the same on both -- the excess over a pairwise table grows with the number of slices -- and gfx1201's is far steeper. But a quarter of gfx1201 is **8** units and a quarter of gfx90a is **26**, so comparing at equal die *fraction* is also comparing at unequal absolute width, and the two cannot be separated by this sweep. That is the same entanglement 1.10 found in the quota curves themselves. |
| **Reproduced across a GPU reset** | The sweep ran twice, on either side of a driver hang that needed a `gpureset`. 1 way 1.048 then 1.050; 2 ways 1.396 then 1.367; 4 ways 2.341 then 2.318. |
| **Control** | One way is a solo and reads **1.050** on gfx1201 and **1.024** on gfx90a -- that is the noise floor, and it is the card warming over six episodes rather than nothing. At **two** ways the arrangement *is* the pairwise one, and gfx90a reads **1.2471** against the call-level table's **1.2176**: two independent instruments on the same arrangement, agreeing to **2.4%**. That agreement is what licenses reading the four- and eight-way divergences as findings. |
| **The allocator is not the cause, and this was checked rather than assumed** | Every cell measures the solo again after the episodes, again after `torch.cuda.empty_cache()`, and again with the peer streams destroyed, and every one returns to the pre-episode solo. Means: 108.5 → 110.5 → 110.1 at one way, 153.4 → 156.6 → 181.2 at two, 262.8 → 269.7 → 267.2 at four, 502.2 → 503.6 → 501.7 at eight. The 181.2 is one noisy pass in which a single slice read 205.3; its `after` control is clean and it is left in rather than dropped. |
| **Scope** | N slices of one model with no other tenant, in **one process** -- across processes there is no partitioning at all, see 1.13. The mismatched arrangement is a different and much larger number -- see 1.12. |
...
| | |
| --- | --- |
| **Claim** | SDXL at 52 units beside **CogVideoX-2b** at 52 pays **4.93**, where the same slice beside another SDXL pays **1.247**. That is **3.95x**. The video tenant pays **1.02** -- it is not a shared cost, it is one-directional. |
| **Evidence** | gfx90a, idle machine, one process, six episodes, `experiments/probes/gfx90a/walled_final/`. Solo 182.4 ms (SDXL) and 790.8 ms (CogVideoX), both on the measured quota curve. Externality **4.928** and **1.017**. The same-model control at the same widths, same sweep, same card: **1.2471**. |
...
| | |
| --- | --- |
| **Claim** | Two processes co-running on gfx1201 each pay **about 2x their own solo**, whether their CU masks are **disjoint halves** (1.97) or **both the whole die** (2.10). Each process gets about half the wall clock; a disjoint mask does not stop that, it only *also* caps the process at half the units, so it pays twice. Two *threads* of one process on those same halves pay **1.37**. The mask partitions within a process and not across the process boundary. |
| **Evidence** | `scripts/run_amd_xproc_corun.py`, one process per tenant, `ROC_GLOBAL_CU_MASK` per process, default stream, 60 s window, `experiments/probes/gfx1201/xproc/`. Whole die each (`--allow-overlap`, both masks `0xffffffff`): solo 112.8 / 112.6 ms, co-run 236.8 / 236.7, **2.100 / 2.103**. Disjoint `16+16`, two runs: co-run 305.8 / 308.7 and 309.8 / 309.9 -- the co-run agrees to 1.3% -- against solos of 144.6 / 145.6 and 157.1 / 157.7, giving **2.115 / 2.120** and **1.972 / 1.965**. |
| **Which of those two solos to believe** | The second. 157.1 ms is the measured quota curve's value for 16 units to three figures; 144.6 is 8% under it and under nothing else in the sweep. The externality is quoted from the run whose baseline lands on the curve, and both are printed rather than averaged. |
...
| **What it costs, in aggregate step rate for both tenants together** | one process alone on the whole die **8.87/s**; two threads of one process at `16+16` **9.54/s**; two processes with the whole die each **8.45/s**; two processes on disjoint halves **6.46/s**. Masking across processes costs **24%** against not masking, because each process is capped at half the units and time-sliced anyway. The ratios above are against different baselines -- 16 units against 32 -- so this row and not those ratios is the comparison. |
...
| **Falsifier** | A cross-process arrangement whose disjoint-mask *aggregate step rate* is at or above the whole-die-each rate. That would mean the mask is buying separation rather than only cost. |
