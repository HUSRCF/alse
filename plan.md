...
- **已决定（2026-09-03，用户）：第二 SKU 加回来。**「还是决定把 SKU 加回来，
  你看看 mi250 那里能不能跑实验」。这与 2026-08-09/08-26 的单 SKU 决定冲突，
  按本文 §五 第 6 条以用户指令为准，变更与理由写入 decision log
  （`docs/gate-c-decision-log.md`，2026-09-03 与 2026-09-04 各条）。
  第二 SKU 是 **DiamondHill，8× MI250X（gfx90a, CDNA2，每 GCD 104 CU）**，
  不是 NVIDIA。所以：
  - **恢复的**：跨架构一致性作为证据。已产出 1.10（quota 曲线在 gfx90a 上
    **证伪 Amdahl 形式**：拟合串行项为负，`q·t(q)` 从 13 到 26 单元下降；
    留一 MAPE 24.9%/28.1%，对 gfx1201 的 5.6%/4.6%，而 Gate B 的门槛是
    10%）、pairwise externality 表（与 gfx1201 在每个 die 份额上差 ≤0.039
    绝对、≤3.6% 相对——**争用会迁移、缩放不会**），以及 3.8 在两个架构上
    都成立（最好切分超 deadline 29.2% / 48.4%）。
  - **未恢复的**：Gate B 的 NVIDIA 条文（4090、四模型、SM quota {16..128}）
    仍然撤销且仍未满足；本决定加的是 **AMD 的第二个架构**，不是 NVIDIA。
    第 13–14 周原「在第二 SKU 运行预注册的 12-cell 精简矩阵」仍为撤除状态，
    因为 gfx90a 上**尚无任何调度 cell 运行过**——前置的两张成本表刚测完，
    是否跑该矩阵是下一个决定，不在本条内。
...
| Gate B-AMD co-run（CogVideoX-2b） | `run_amd_inproc_corun.py --share-weights`（单进程双 masked stream，共享一份权重：14.7 GB 对双进程的 57 GB） | `inproc_cogvideox2b_9f_shared_v2_20260808.json`：3 trial、重叠 111.1 s（占窗口 92.5%）、mask 不相交、externality a=+27.2% / b=+31.5%；穿透性前置证据 `stream_mask_penetration_20260807.json` | **exact** | 条文 2026-08-08 修订为允许 stream 形态**但强制附穿透性证据**；无证据者记 NOT_MEASURED。11 项测试守住该前置条件 |
| Gate B-AMD co-run（CogVideoX-2b） | `run_amd_inproc_corun.py --share-weights`（单进程双 masked stream，共享一份权重 14.7 GB） | `inproc_cogvideox2b_9f_shared_v2_20260808.json`：3 trial、overlap 92.5%、masks disjoint、externality +27.2%/+31.4%；穿透性证据 `stream_mask_penetration_20260807.json`。verdict `gate_b_amd_verdict_cogvideox2b_20260808.json` **9/10**，唯一 FAIL 为 `cold_model`（与 SDXL 相同，推迟至 runtime 阶段） | **exact** | 条文已于 2026-08-08 修订为接受 stream 形态，**但以 mask 穿透性证据为强制前置**；无证据或证据呈泄漏趋势者一律 NOT_MEASURED（已用伪造的泄漏证据验证该前置确实生效） |
...
- co-run cell 可用**两个各带不相交掩码的进程**，或**单进程内两个各带不相交
  掩码的 stream**。**必须证明两侧确有时间重叠**：不重叠的两次运行会给出与
  「零外部性」完全相同的数字。取证方式为 warmup 后 barrier 同步、按固定墙钟
  时长而非固定样本数采样，且只统计完全落在双方共同采样窗口内的样本；跨窗口
  边界的样本一律丢弃而不按比例折算——折算会把外部性稀释向零，即让结果好看的
  方向。

  > **2026-08-08 修订**：原条文规定「两个进程，而非单进程多 stream」，理由是
  > per-stream 掩码可能与框架内部发射耦合——进程级 `ROC_GLOBAL_CU_MASK` 约束
  > 进程发射的每一个 kernel，而 stream 掩码只约束真正到达该 stream 的 kernel，
  > 若框架把部分工作发到 default stream，分区就只是名义上的。**该风险现已实测
  > 排除**：`stream_mask_penetration_20260807.json`——单进程单 masked stream 的
  > per-step 曲线对照进程级掩码曲线，4/8/16/32 units 的比值为
  > 0.9900 / 0.9887 / 0.9847 / **0.9739**。**判据在实验前写定且具方向性**：
  > 掩码泄漏在配额最紧处收益最大，故泄漏会使低配额格外偏快（4 units 被拉向全
  > die 的 115 ms）；实测趋势**相反**——低配额最吻合、高配额偏差最大，残差集中
  > 在步长最短处，与双进程 harness 的固定开销（subprocess 启动与同步）一致，
  > 属 harness 差异而非掩码差异。
  >
  > 因此允许 stream 形态，**但附带强制前置条件**：凡以 stream 形态取证的
  > co-run cell，必须同时提供该卡该框架版本下的 **mask 穿透性证据**（单 stream
  > 逐配额 per-step 曲线对照进程级掩码曲线，且偏差不得随配额降低而放大）。
  > 无此证据的 stream 形态 co-run 记为 NOT_MEASURED，不得记为 PASS。
  >
  > 修订的直接动因是 CogVideoX-2b：双进程需 57 GB 而卡仅 34.2 GB，**在原条文
  > 下永远不可测**；单进程共享一份权重仅需 14.7 GB。修订的依据是上述实测，
  > 不是「否则测不到」——后者只说明条文挡住了什么，不说明条文错了。
...
> **2026-08-10(补)——失配配对已用跨度仪器重测,撤回的数值全部恢复,瞬态定性为假象。**
> 五 split × 5 进程 × 3 episode,solo 与 co-run 同源跨度,每条 15 次测量、
> 极差 ≤0.005:
>
> | split | SDXL ext | CogVideoX ext | SDXL vs 轮转 | CogVideoX vs 轮转 |
> | --- | --- | --- | --- | --- |
...
>
> **五个 split 上两侧同赢**,数值与撤回前相差数个百分点——原"稳定值"是对的。
> 八条暂停条目已解除并更新。
>
> **"两个 episode 串行化瞬态"定性为仪器假象**:跨度仪器逐 episode 平坦到
> 三位小数(4+28 为 1.025/1.025/1.024)。那个跨五 split 拟合到 3–9% 的
> `solo_a+solo_b` 拟合的是一个每 episode 只持有一个陈旧值的变量。

> **2026-08-10——探测的价值已测定,但它的机制与当初的论证不同。**
> 同模型共置(两个 SDXL 租户,768),三臂分解,按进程抽到的态拆分:
>
> | | 慢态 n=10 | 快态 n=33 |
> | --- | --- | --- |
> | deadline 动作 | −0.57% 跨零 | +0.62% 跨零 |
> | **探测** | **−10.87% [−0.0635,−0.0083]** | **−9.75% [−0.0292,−0.0112]** |
> | 合计 | −11.37% 排除零 | −9.19% 排除零 |
> | 串行回退 | 1341→448(−67%) | 5498→2158(−61%) |
> | video goodput | +0.03% | +0.04% |
>
> **探测在两个态里都成立、量级相同、机制相同;deadline 动作两个态里都是零。**
> 这否定了"探测是双稳态检测器"这一设计论证:若如此,效果应集中于慢态。
> 实测它与状态无关,做的是**用一次便宜的早期纠正替掉一次昂贵的整卡串行兜底**。
> 论文应按后者描述它。
>
> 慢态抽取率合并 **27/160 = 16.9% [11.4%, 23.6%]**;campaign 间仍不稳定
> (8.3% / 11.7% / 22.7% / 35.0%),预测候选已死五个。但**该率不再影响主张**,
> 因为主张不再依赖抽到哪个态。
>
> **范围限定(不得省略)**:以上是**同模型共置**。plan.md 的主矩阵是
> SDXL × CogVideoX 失配配对(实测 1.02/1.05、无抽取),那里探测从不触发、
> 180 cells 贡献精确为零。且 10% 仍低于 plan.md 的 20% 门槛。

> **2026-08-09 第五次修订(收敛)——solo 也换成同一仪器,判决闭合。**
> 6 进程,solo 与 co-run 全部由设备跨度测得:
>
> | 态 | 进程 | ext | solo16 | solo32 | 配对 | 轮转 | 分区 |
> | --- | --- | --- | --- | --- | --- | --- | --- |
> | 快 | 5/6 | 1.299 | 154.6 ms | 112.0 ms | 200.9 ms | 224.1 ms | **+11.5%** |
> | 慢 | 1/6 | 1.945 | 155.4 ms | 112.6 ms | 302.4 ms | 225.2 ms | **−25.5%** |
>
> 两次跨度 campaign 合计 **12/14 进程抽到快态**。把 solo 也换成同一仪器带来的
> 修正是:solo32 由 108.9 变为 112.0 ms(+2.8%),这**抬高了轮转基线、于分区
> 有利**——明说而不揣兜里:同样的跨度 externality,用旧基线算是 +8.5%,用同一
> 仪器的基线算是 +11.5%。
>
> `MEASURED_STEP_PAIRING_FAST/_SLOW` 已解除暂停并改为 1.299 / 1.945,抽取率
> 12/14;`MEASURED_STEP_PAIR_EXTERNALITY` 中 16+16 的两条改为跨度值 1.022 /
> 1.054,**其余八条仍然暂停**——只有 16+16 被重测过。

> **2026-08-09 第四次修订——撤回范围已收窄,双稳态在受检仪器上重新确立。**
> 用"在同一条流上包住该步"的设备跨度重测(8 进程 × 4 episode,同构 16+16):
>
> | | 进程 | 侧·episode 数 | 均值 | 区间 | overlap |
> | --- | --- | --- | --- | --- | --- |
> | 快态 | 7/8 | 56 | **1.274** | 1.238–1.328 | 0.91–0.96 |
> | 慢态 | 1/8 | 8 | **1.871** | 1.843–1.920 | 0.994 |
>
> 比值 1.468,**按进程锁死**(p6 四个 episode 全慢,其余七进程全快,无跨越)。
> 与适配器读数给出的 1.176/1.776(比值 1.51、6/8 快)相比:**形状与锁死性质
> 成立,绝对值不成立**。重算收益:**快态 +8.5%、慢态 −26.1%**(原为 +13.0%/−25.9%)。
>
> **仍然撤回**:失配配对的"两个 episode 瞬态"、跨五 split 的 `solo_a+solo_b`
> 拟合、各 split 稳定表、24+8 的 3:1 离群态,以及两组常数的绝对值——它们都是
> 一个"整个 episode 只持有一个值"的变量的中位数。
>
> **另行确立**:失配配对确实几乎免费(1.02 / 1.05,无跨 episode 瞬态);
> 慢步真实存在,但发生在**每个 episode 内部的开头**(1684 1683 888 后转 156),
> 四个 episode 形状相同,不随 episode 衰减。

> **2026-08-08 深夜第三次修订——以下整段的 per-step co-run 数值全部暂停。**
> 本项目所有 per-step co-run 数字都取自适配器的延迟事件读数
> （`adapter.last_step_seconds`）。同一进程内用"在同一条流上包住该步"的事件
> 跨度作对照：适配器的区间被跨度**包含**，读数在物理上不可能更大，但失配配对
> 的 ep1/ep2 上跨度是 1.022 而适配器是 **10.888 / 6.234**——包含关系被破坏
> 十倍，说明适配器报的不是它被归属的那一步。原因是延迟读只在上一步事件退休后
> 才更新，不更新时旧值会被反复计入，一次慢读即可占据中位数。
>
> **因此暂停（待重测）**：双稳态与按掩码对锁死、两个 episode 的瞬态及其
> "串行化"解释、`MEASURED_STEP_PAIR_EXTERNALITY`、`MEASURED_STEP_PAIRING_FAST/_SLOW`、
> 以及 +41.7%、+13.0%/−25.9% 等全部数值。围绕它们写的代码（sticky 策略、
> latched 建模、串行回退）本身不因此为错，但其**标定失去依据**。
>
> **不受影响**：一切靠逐位比较或计数得到的结论（Gate A 掩码、与 ASLE 逐位
> 一致、residency、泄漏、10,000 次切换、Jain），以及 solo quota 曲线——那些
> harness 每次新建适配器，首步必然强制排空。
>
> 跨度仪器暂时给出（3 进程，未采纳）：失配 16+16 为 1.02/1.05 且**无瞬态**；
> 同构 16+16 为 1.23–1.35 且**无双峰**，overlap 0.90–0.92。跨度含主机发射
> 时间，是上界。

> **2026-08-08 重大更正（当日晚间再修订）：下述 +18.6% 不成立，但取代它的也不是一个数，而是一个状态。**
...
> 以及建立其上的利用率主张。
>
> **当日晚间续：per-step externality 是双稳的，而且会锁死。** 同一脚本、同一
> 流数的两个进程，在进程内第一次 co-run 上完全一致（ext 1.51/1.51），随后
> 朝**相反方向**跳到两个平台并不再变动。按"进程内第几次 co-run"归并全部
> 16+16 测量：
>
> | | n | ext 均值 | 区间 | 分区收益 |
> | --- | --- | --- | --- | --- |
> | 进程内首次 co-run | 22 | **1.541** | 1.419–1.614 | **−13.2%** |
> | 之后·快态 | 4 | 1.149 | 1.127–1.170 | **+16.4%** |
> | 之后·慢态 | 4 | 1.728 | 1.690–1.762 | **−22.6%** |
> | 五 split harness | 6 | 1.808 | 1.760–1.856 | −25.5% |
>
> 首次 co-run 是本项目最可复现的数字（sd 0.043，跨 12 个独立进程）；之后
> 落在哪个平台是**按进程抽取并锁死**的。因此上表的 −12.8% 是"首次"这一态，
> 不是"那个值"——**推翻的力度被收回，方向没有**。1.2367 也没被救回：它是
> call 口径，且恰好落在几个态之间而不描述其中任何一个。
>
> **确立的是一个更强的陈述：该硬件上的空间分区没有吞吐常数。** 单一
> externality 常数的成本模型，是在对一枚已经落地、但它看不见的硬币做标定。
> 这恰好是双账本存在的理由：按预测计费会让账面自洽且失明，而 runtime 按
> 实测计费，`probing_partitioning` 在实测超出预测容差时即停止配对——会测量
> 的调度器能骑住任一状态，按常数标定的不能。
>
> **待测（以上均未回答）**：两态各自的抽取概率、是否会在进程内翻转；判别
> 状态需要多少步（probe action 依赖这个数要小）；锁死是否跨掩码对存活。
> 失配配对 SDXL × CogVideoX 的适配器与 harness 已写好并同步，但在状态问题
> 定下来之前跑它，只会再产出一个无人能解释的数。
>
> **当日深夜:失配配对测出来了,结论反过来。** SDXL(768)× CogVideoX-2b
> (480×720×9f),16+16 不相交掩码,每进程 5 个 episode,4 个进程:
>
> | episode | SDXL ext | SDXL vs 轮转 | CogVideoX ext | CogVideoX vs 轮转 |
> | --- | --- | --- | --- | --- |
> | 1 | 6.29 | −77.3% | 1.046 | +22.9% |
> | 2 | 6.15 | −76.8% | 1.051 | +22.4% |
> | 3–5 | **1.010** | **+41.7%** | **1.051** | **+22.3%** |
>
> **五个 split 全部测过,稳定态一律两侧同赢**(SDXL 侧 +2.6% 到 +67.9%、
> CogVideoX 侧 +1.7% 到 +30.6%,split 只决定收益如何分配)。瞬态期两侧步时
> ≈ 两个独占步时**之和**(五个 split 误差 3–9%),即前两个 episode 完全串行、
> 第三个起才真正并发。24+8 复现三个进程后另见一个**量化**的离群态:SDXL 一步
> 恰好跨对端三步(4699.8 / 4701.2 ms,两次相差 0.03%),14 个稳定 episode 中
> 出现 2 次,下一个 episode 即自行恢复——这正是 runtime 侧 drift>15% 串行回退
> 加退避所处理的形态。
>
> 稳定后两个租户各占半个 die,却几乎都跑在自己的独占速度上——**分区在这里
> 帕累托优于轮转**:同样时间 T 内 SDXL 走 6.41T 步(轮转 4.56T)、CogVideoX
> 走 1.198T 步(轮转 0.98T),没有一方在为另一方的收益买单。而且**没有抽取**,
> 4 个进程的曲线彼此相差不到 0.02。
>
> **于是本项目的核心主张应当改写为:**
>
> - **失配租户**:分区显著划算(+41.7% / +22.3%),确定性,可复现。
> - **同构租户**:双稳且按掩码对锁死——快态 1.176(+13.0%)、慢态 1.776
>   (−25.9%),6/8 进程抽到快态,一步即可判别,从不翻转。
> - 因此"分区提高利用率"不是一个常数,而是**取决于配对构成与一次抽取**;
>   能测量的调度器可以骑住它,按常数标定的不能。这比原来的 +18.6% 弱,
>   但它是真的,而且让双账本从装饰变成承重结构。
>
> **两个 episode 的瞬态是对刚建成的动作的陷阱**:SDXL 前两个 episode 是
> 6.29×,而 probe 的阈值是 1.608×——它会在第一步就丢掉那个三个 episode
> 之后能给 +41.7% 的配对。冻结策略每轮重组,所以付出两个 episode 后能找到;
> **永久性的粘性判决则永远找不到**。这正是早上那个只凭"未被排除"写下的
> 退避机制,现在有了实测依据。
...
> **2026-08-08 作废**：上面这个 1.1862 是 simulator 在 call 级 externality
> 常数 1.2367 下的输出。真机 per-step 测量表明该常数不描述任何一个实际状态
> （进程内首次 1.541、快态 1.149、慢态 1.728），**分区收益随状态在 +16.4%
> 与 −22.6% 之间翻转**。此数不再作为结果引用，详见本文件 460 行处的更正块与
> `docs/gate-c-decision-log.md`。Gate C 七条本身不受影响——它们断言的是
> "给定成本模型下调度器是否稳健"，而非绝对吞吐增益。
...
> **2026-08-06 新增限定（最终）：双稳态属于"双进程测量 harness"，目标
> runtime 形态（单进程双 stream）不存在该现象。**
>
> 此前曾记录 co-run externality 呈双峰（快态约 30%），并为此设计"探测-
> 重组"。用**设计所规定的形态**——单进程、单 context、两个带不相交 CU mask
> 的 stream——重测 **17 个 trial（两次运行 5 + 12），全部为快态**：
> +21.1~28.7%，24 侧均值 **1.2367**，与双进程快态 1.2362 相差 **0.04%**。
> 若快态率真为 30%，17 次全快的概率为 **1.3×10⁻⁹**。
>
> **因此：双稳态是双进程测量 harness 的产物，不是这张卡的性质**，runtime
> 不继承它。此前四次因果解释（工作集阈值 / 功耗上限 / 不可控双稳态 / 启动
> 错开）都是在"它是硬件性质"这一错误前提下做出的。
>
> **已确立**（**2026-08-08 作废前两条，见下**）：
> - ~~目标形态下 externality **稳定为 1.2367**（24 侧测量，逐 trial cv
>   0.39–0.54%），Gate C 分区收益 **+18.6%**。~~ 该 1.2367 测的是整次
>   `pipeline(**call)`（含 VAE decode），而调度器按 per-step 决策；per-step
>   下同一配对给出 1.541 / 1.149 / 1.728 三个态。
> - ~~双进程形态确有双峰……单进程 17/17 取快态。~~ 双峰在**单进程掩码流形态
>   下同样存在**：两个同脚本进程在首次 co-run 上一致（1.51/1.51）后朝相反
>   方向锁进 1.149 与 1.728，且此后不再变动。原文"单进程 17/17 取快态"是
>   call 级口径下的观测，不适用于 per-step。
> - simulator 的 `PairingStates(enabled=True)` 机制仍然适用，但其抽取粒度
>   （每个配对独立抽）与实测（**每进程抽一次并锁死**）不符，待重新参数化。
>
> **探测策略保留但在目标形态下为空操作**（已断言）。保留理由：本项目多数
> co-run 证据采自双进程形态，且以独立进程部署租户的场景确有该问题——彼时
> 探测得 1.133、盲目配对得 0.895。
>
> 探测本身暴露并修复了两个缺陷：(1) 它原先拿"含配对惩罚的实测"去比"不含
> 惩罚的 solo 预测"，快态本就相差 1.24×，1.4 阈值仅余 13% 裕度，−20% 预测
> 误差即越界（触发 106 次伪独占）；已改为与配对预期比较。(2) ±20% 误差下
> 两态判别带**数学上重叠**（快态被低估读作 1.25×，慢态被高估读作 1.20×），
> 任何阈值都无法兼顾；`slow_factor=1.3` 使其**向"不动作"收敛**——误差超过
> 约 10% 后探测退化为空操作，策略行为等同 `slo_aware_partitioning`。
> **对噪声过大的预测采取行动，正是探测变成损害的方式。**
>
> **方法约束**：单次 co-run 不构成证据；externality 结论必须基于重复测量的
> 分布；**改变任何参数之前，先测固定参数下的基线分布**；**测量所用的进程/
> stream 架构必须与被声称的部署形态一致**——本轮四次错误解释的根源正是
> 用双进程 harness 去推断单进程 runtime 的性质。
...
**一个曾被误判为策略缺陷、现已撤回的结论**：约 0.60 req/s、到达 seed=11
时 `deadline_aware` 的 miss 数（35）多于完全不干预的策略（26）。这一度被
记为策略缺陷并固定成测试。**该结论不成立**——在 0.60 下换 10 个到达 seed
检验，干预策略在其中 **9 个**上更优，seed=11 是尾部个例，那个读数测的是
到达序列而非调度器。两次针对该"缺陷"的修复尝试都只改善了那一条 trace 而
让所有更高负载变差（1.30 req/s 下 117 → 136 / 134，而不干预是 138）。
predictor 降级场景的负载也据此从 0.45 改为 0.80：跨 10 个 seed，
informed 优于 blind 的比例在 0.45 为 3/10、0.60 为 9/10、0.80 为 10/10，
只在部分抽样上可区分的负载不是更严的测试而是更吵的测试。
...
>
> **完整 loop 已在真机跑通（2026-08-08）**：R9700 上两租户各 2 请求、
> 每请求 6 步、SDXL 768×768、共享一份权重、冻结策略
> `probing_partitioning` 原样驱动真实 executor。证据
> `runtime_loop_v4_20260808.json`：
>
> | 验收项 | 实测 | 判定 |
> | --- | --- | --- |
> | scheduler 决策 p99 < 1 ms | p50 **10 µs** / p99 **17 µs** / max 17 µs | **通过（余量 55×）** |
> | 同模型 burst residency 后不传权重 | 首轮后 **0 bytes** | **通过** |
> | 记账来自实测而非成本模型 | `charged_from_measurement = True` | **通过** |
> | backlogged Jain ≥ 0.98 | t0 20.707 / t1 20.839 → **0.99999** | **通过** |
>
> 双账本可用:预测 vs 实测的偏差 mean −0.313(24 个样本),即成本表在该
> 工作点系统性高估约 31%——偏差可见正是双账本存在的理由。
>
> 过程中修掉三个**测量伪装成策略行为**的缺陷(详见 decision log)：
> (1) adapter 每步 `synchronize` 排空流水线，把排空代价算进被测步长
> （0.1155 s 读成 0.256 s）；(2) probe 拿"独占时的观测"或"含 kernel 编译
> 的首步观测"(1.855 s vs 稳态 0.157 s)去判定配对质量，导致请求被永久
> 排除出配对——**冻结算法已据此修正**(post-freeze #9)；(3) 一次性的 kernel
> 编译成本被记入首个到达租户的账，使其账本为对方的 10.8 倍——现记入
> residency。修复后总轮数由 21–23 降至 **12**，配对真正生效。
>
> **一小时压测通过（2026-08-08）**，证据 `soak_3600s_v2_20260808.json`：
> 完整 3600 s 窗口、Poisson 到达、**admitted 714 / completed 714**、
> peak backlog 4、**failure: None**。
>
> | 验收项 | 实测 |
> | --- | --- |
> | 无泄漏 | slope **−0.0000 MB/s**；每完成请求 **+0.0000 MB**；alloc 首末皆 6.644 GB |
> | 无死锁 | 无"有待办却零授予"的连续轮次；4182 轮全部在 watchdog 内 |
> | 无 OOM | 无 |
> | scheduler p99 < 1 ms | p50 **3 µs** / p99 **11 µs**（n=4182） |
> | backlogged Jain ≥ 0.98 | t0 7137.1 / t1 7565.1 → **0.99915** |
>
> **首次压测是失败的，两个缺陷都由它暴露**：(1) runtime 永久保留已完成
> 请求的 executor，其 adapter 持有 GPU 上的 conditioning 张量，**每完成一个
> 请求泄漏 1.284 MB**（372 个请求 = +478 MB），且泄漏量跟随**完成数**而非
> 时钟——这正是"泄漏"与"allocator 稳定过程"的判别依据；(2) 压测脚本在无
> 工作时空转，把 386 个请求变成 **1.24 亿轮**与同样多的 ledger 记录，最终
> 超时的那一轮慢在 ledger 而非 GPU。泄漏判据也据此从"字节/秒"改为
...
- ~~在第二 SKU 运行预注册的 12-cell 精简矩阵。~~ 撤除（单 SKU 决定）。
  **2026-09-03 用户把第二 SKU 加回来后仍为撤除状态**，且这是刻意的：第二
  SKU 是 gfx90a 而非 NVIDIA，目前只测了两张成本表（1.10 的 quota 曲线与
  pairwise externality 表），**没有跑过任何调度 cell**。要不要在那里跑精简
  矩阵是一个新的决定，需要它自己的预注册；把撤除条自动复活会让"第二 SKU
  验证过"这句话在没有调度证据时成立。
...
## 四之二、主张与证据(2026-08-12 新增)

`docs/claims-and-evidence.md` 是**扁平视图**:每条主张一行,给出证据 run、
作用域、以及什么会推翻它。决策日志是编年体且已极长,本文件叠了多层更正,
两者冲突时**以 claims-and-evidence.md 为准**(它更晚)。

它同时是第 15 周"定理、局限与 threat-to-validity 文稿"条款的交付物,
以及第 16 周投稿包的骨架。
...
- 2026-08-05 / power-tracks-the-mask-and-peaks-at-24-units：**⚠️ 本条结论已于
  同日被 `power-is-pinned-at-the-cap` 推翻,保留原文以存证,勿引用其能效结论。**
  错误根源:所用负载(8192³ 四次链式 matmul)受显存带宽限制,功耗未达功耗墙,
  因而呈现「功耗随配额上升」的假象;换用饱和负载后功耗恒定。原文如下——
  **功耗是掩码生效的
  第三条独立证据,且给出一个能效最优配额**。起因是注意到 `rocm-smi` 的
  `GPU use (%)` 恒为 100——**该指标衡量「设备是否有工作在执行」的时间占比,
  不是有多少 CU 在忙**,故 4 单元与 32 单元同报 100%,在掩码实验中无信息量。
  改测板级功耗(8192² fp16 matmul 链,每配额持续 12 s):
  | 单元 | 功耗 | TFLOPS | 能效 (TFLOPS/W) |
  |---|---|---|---|
  | 4 | 106 W | 16.78 | 0.158 |
  | 8 | 110 W | 26.85 | 0.244 |
  | 16 | 120 W | 43.44 | 0.362 |
  | 24 | 135 W | 51.17 | **0.379**(峰值) |
  | 32 | 150 W | 53.95 | 0.360 |
  **(1) 功耗随配额单调上升**,与读回值、与计时都不同源,构成第三条独立证据。
  **(2) 固定开销约 100 W**——线性外推至 0 单元得 99.7 W,即**满载 150 W 中
  有 67% 不随掩码缩减**(显存控制器、IO、时钟树、L2)。这与 co-run 的
  23% 外部性是**同一块共享硅片**的两种表现:掩码分区的只是 CU。
  **(3) 边际能效急剧衰减**:4→8 单元 **2.52 TFLOPS/W**、8→16 **1.66**、
  16→24 **0.515**、24→32 **0.185**——最后 8 个单元的边际能效是最初 4 个的
  **1/13**,且 32 单元的总能效(0.360)**低于** 24 单元(0.379)。
  **对调度器的直接结论**:回收 24 以上的配额几乎不损失吞吐(24→32 仅
  +5.4% TFLOPS)却省 11% 功耗,**24 单元是能效最优点**。这与「高配额延迟
  边际收益递减」是同一现象的两面,但功耗给出了更硬的取舍依据。
  **harness 变更**:每个 cell 现在记录 `package_power_w`(负载中采样)与
  `idle_package_power_w`,使功耗成为常规证据而非一次性诊断。
- 2026-08-05 / power-is-pinned-at-the-cap：**⚠️ 「功耗与配额无关」一句由
  `power-wall-starts-at-12-units` 修正——功耗墙自 12 单元起才成立,4/8 单元
  未达墙。「32 单元能效最优」的主结论不变。原文如下——**
  **推翻上一条。卡始终贴在功耗墙上,
  功耗与配额无关;能效严格随配额单调上升,32 单元最优(而非 24)。**
  起因是使用者指出「没把功耗榨干说明还有提升空间」——这个质疑是对的,
  而且暴露了此前**两处**测量错误。
  **(1) 负载未饱和**:此前用 4 次链式 matmul(`x = mm(x, b)` 重复 4 次),
  中间结果反复写回显存,实为**带宽受限**;单次 4096³ matmul 才是计算受限。
  实测差距:链式 **53.95 TFLOPS / 150 W** vs 单次 **122.58 TFLOPS / 300 W**——
  此前只用到了真实算力的 **44%**、功耗的 **50%**。
  **(2) 功耗采样漏峰**:此前在迭代间隙采样,慢 kernel 采样稀疏;改为后台线程
  定频采样(0.25 s)后才读到真实峰值。
  **重测(单次 4096³ fp16 matmul,每配额 10 s,后台采样)**:
  | 单元 | TFLOPS | 功耗中位 | 能效 (TFLOPS/W) |
  |---|---|---|---|
  | 4 | 12.38 | 299 W | 0.041 |
  | 8 | 44.59 | 285 W | 0.157 |
  | 12 | 61.85 | 299 W | 0.207 |
  | 16 | 76.69 | 299 W | 0.257 |
  | 20 | 89.28 | 299 W | 0.299 |
  | 24 | 100.33 | 300 W | 0.334 |
  | 28 | 110.36 | 300 W | 0.368 |
  | 32 | **122.32** | 299 W | **0.409**(峰值) |
  **对调度器的结论(与前一条相反,以本条为准)**:
  (a) **切分配额不省电**——4 单元与 32 单元同样吃约 300 W,GPU 会拉高时钟直到
  撞墙,空闲 CU 省下的功耗被时钟提升吃回去;
  (b) **切分配额严重损失能效**——4 单元的能效仅为 32 单元的 **1/10**;
  (c) 故**能耗最优策略是让单租户用满**,这与多租户切分的诉求直接冲突,
  **该权衡必须由调度器显式承担**,不能假装配额切分是免费的。
  (d) 4 单元为异常点:12.38 TFLOPS 远低于由 8 单元(44.59)线性外推的值,
  4→8 单元吞吐增 **3.6 倍**而配额仅增 2 倍;8→32 则为 2.74 倍(4 倍配额),
  正常次线性。极低配额下的额外损失成因待查。
  **方法学教训**:「负载是否饱和」必须以**功耗是否触及功耗墙**为判据之一,
  仅凭「更大的问题不提高吞吐」会把带宽受限误判为算力饱和。Gate B 的饱和标注
  据此仍然有效(它判定的是 CU 是否为该 cell 的瓶颈),但**不得据此推断卡已满载**。
- 2026-08-05 / partitioning-costs-aggregate-throughput：**回答「功耗恒定是否
  只是单租户假象」——不是。切分配额确实损失总吞吐,且损失量有明确形状。**
  此前的配额扫描测的是「一个进程持有 N 个单元、其余 32−N 空闲」,那不是
  多租户的样子。正确的问法是:**K 个租户各持 32/K 个不相交单元,合起来
  能否达到单租户独占 32 个单元的吞吐?** 实测(4096³ fp16 matmul,
  各租户同时起跑,共同窗口 10 s):
  | 租户 × 单元 | 总 TFLOPS | 相对独占 | 功耗 | 每租户 |
  |---|---|---|---|---|
  | 1 × 32 | **123.76** | 100% | 300 W | 123.8 |
  | 2 × 16 | 80.87 | **65%** | 300 W | 40.4 |
  | 4 × 8 | 80.01 | **65%** | 299 W | 20.0 |
  | 8 × 4 | 42.49 | **34%** | 298 W | 5.3 |
  **三条结论**:
  (a) **切分有实实在在的总吞吐代价**,与「同时只有一个用户在跑」的假象无关——
  8 个租户把 32 个单元全部占满,总吞吐仍只有独占的 34%。
  (b) **代价的形状是阶梯而非渐进**:1→2 租户损失 35%,2→4 租户**不再恶化**
  (65% → 65%),4→8 租户再腰斩至 34%。**故 4 并发租户是本负载下的拐点**,
  调度器的并发度上限应据此设定,而不是任意切分。
  (c) **功耗全程钉在 ~300 W**,与租户数和配额均无关,故**切分既不省电也不
  多耗电**;代价纯粹体现在吞吐上。
  与 co-run 外部性的关系:2 租户各 16 单元时每租户 40.4 TFLOPS,而单进程
  16 单元为 76.69——**每租户损失 47%**,远高于 SDXL co-run 实测的 23%。
  差异符合预期:纯 matmul 是计算密集的,对共享的 L2/带宽/命令处理器竞争
  最激烈;diffusion 含更多非计算环节,竞争被稀释。**externality 是负载相关的,
  pairwise table 必须按负载类型分别标定,不能用单一系数。**
  待查:4 单元为异常低效点(单进程 12.38 TFLOPS = 3.10/单元,而 8 单元为
  5.57/单元),8 租户×4 单元时更降至 1.33/单元。疑为 tile/occupancy 不足,
  须用硬件计数器(wave occupancy、CU 忙碌周期)确认,而非继续从吞吐反推。
- 2026-08-05 / occupancy-telemetry-and-the-low-quota-anomaly：**⚠️ 本条中
  「低配额异常」的部分已被 `the-low-quota-anomaly-was-measurement-interference`
  推翻——该异常不存在。占用率遥测部分仍然有效。原文如下——**
  针对「加入设备
  利用率」与「低配额未吃满」两问的调查结果,**部分定论,部分未决,分别标明**。
  **(1) 占用率的派生计数器在 gfx1201 上不可用**——`OccupancyPercent`、
  `MeanOccupancyPerCU`、`MeanOccupancyPerActiveCU` 全部返回 **0.00**,
  且 `rocprofv3` 采集时段错误(仍产出文件);`GPUBusy` 恒为 100%,
  与 `rocm-smi` 的 `GPU use (%)` 同样无信息量。**这与 PCIe 遥测不可用
  (2026-08-03)属同类平台限制**:RDNA4 工作站卡的派生遥测支持不完整。
  **可用的基础计数器**(实测有效):`SQ_WAVES`、`SQ_BUSY_CYCLES`、
  `GRBM_GUI_ACTIVE`、`GRBM_COUNT`。故「设备利用率」只能由基础计数器构造,
  不能直接读取——这一点须写进 harness,否则会像 `GPU use (%)` 一样
  记录一个恒定值并误以为是利用率。
  **(2) 低配额异常的部分定位**(4096³ fp16 matmul,4 单元 vs 32 单元):
  | 量 | 4 单元 | 32 单元 | 比值 |
  |---|---|---|---|
  | `SQ_WAVES` | 2133 | 2432 | ~1.0 |
  | `GRBM_COUNT`(kernel 周期) | 11.39 M | 1.91 M | **5.95×** |
  | `SQ_BUSY_CYCLES` | 333.7 M | 58.7 M | 5.68× |
  | 实测吞吐 | 12.38 TF | 122.32 TF | **9.88×** |
  三点推论:(a) **发射的 wave 数与配额无关**,符合预期——同一 kernel、
  同样的工作量,故差异不在工作量划分上;(b) **kernel 执行时长只慢 5.95×,
  而 CU 数差 8×**,即**低配额下每 CU 的效率反而更高**(每 CU 分到的 wave
  更多,延迟隐藏更充分);(c) **但端到端吞吐差 9.88×**,与 kernel 时长的
  5.95× 相差 **1.66×**——**该差额不在 kernel 内部**。
  **未决**:那 1.66× 的去向。候选为 kernel 之间的发射间隙(低配额下单次
  kernel 长,但发射频率受 CPU 侧限制的比例变化)、或功耗墙下的时钟行为差异。
  **须继续用计数器与时间线测量,不得从吞吐反推**——本轮的教训正是反推会
  得出错误结论(见 `power-is-pinned-at-the-cap`)。
- 2026-08-05 / the-low-quota-anomaly-was-measurement-interference：**「4 单元
  异常低效」不存在;它是测量工具污染被测对象造成的假象。而且方向恰恰相反——
  低配额下每单元效率更高。**
  用 `rocprofv3 --kernel-trace` 直测(4096³ fp16 matmul,40 次):
  | | 4 单元 | 32 单元 |
  |---|---|---|
  | 单 kernel 时长 | **6.553 ms** | **1.025 ms** |
  | kernel 间隙 | 0.012 ms(占 0.2%) | 0.012 ms(占 1.2%) |
  | 墙钟 | 0.2627 s | 0.0416 s |
  **(a) 间隙可忽略且两者相同**,故此前「1.66× 差额在 kernel 之外」的猜想
  被排除——**该差额根本不存在**。
  **(b) 真实比值是 6.39×(kernel)/ 6.31×(墙钟),不是此前记录的 9.88×。**
  假象来源:此前的配额-功耗扫描内含一个每 0.25 s 调用 `rocm-smi` 的后台采样
  线程,4 单元那次采得 **552** 个样本而 32 单元仅 **85** 个——**慢 kernel
  被采样干扰得更久**,吞吐被系统性拉低。
  **(c) 修正后的每单元效率:4 单元 5.24 TFLOPS/单元 vs 32 单元 4.19**——
  **低配额下每单元效率更高**,与「4 单元异常低效」的记录相反。成因合理:
  32 单元时贴着功耗墙,时钟被压;4 单元时每 CU 分到更多 wave,延迟隐藏更充分,
  且不受功耗限制。
  **对既有记录的影响——已实测,结论是「无影响」**:去掉采样线程重跑多租户
  实验,四组结果与含采样时**差异均 <0.4%**(1×32:123.75 vs 123.76;
  2×16:80.98 vs 80.87;4×8:79.97 vs 80.01;8×4:42.34 vs 42.49)。
  故 `partitioning-costs-aggregate-throughput` 的数据**完全有效,无需修正**。
  (此处曾推测其偏低约 8%,该推测已被实测否定。)
  **干扰为何只影响配额扫描而不影响多租户实验——机理**:配额扫描的采样线程
  与计算循环**在同一 Python 进程内**,争用 GIL,直接拖慢 kernel 发射;
  多租户实验的采样线程在**驱动进程**,而租户是独立子进程,不共享 GIL。
  故判据是「采样是否与被测代码同进程」,不是「是否在同一台机器上」。
  **方法学教训(第二次)**:遥测采集本身是负载。0.25 s 一次的 `rocm-smi`
  子进程调用足以显著干扰计时,且**干扰量与被测负载的速度相关**,因而会在
  配额扫描中制造出与配额相关的假趋势。今后:功耗与计时**不得在同一次运行中
  采集**;需要两者时,分两次运行,或使用不派生子进程的接口。
- 2026-08-06 / power-wall-starts-at-12-units：用**跨进程采样 + 饱和负载**重测
  配额-功耗曲线(`scripts/run_amd_power_sweep.py`,负载在子进程、采样在父进程,
  单次 4096³ matmul 而非链式),修正此前两处数字:
  | 单元 | TFLOPS | 功耗 | 能效 | 每单元 |
  |---|---|---|---|---|
  | 4 | 22.76 | **184 W** | 0.124 | **5.69** |
  | 8 | 44.53 | **280 W** | 0.159 | 5.57 |
  | 12 | 61.87 | 300 W | 0.206 | 5.16 |
  | 16 | 76.58 | 300 W | 0.256 | 4.79 |
  | 20 | 89.28 | 300 W | 0.298 | 4.46 |
  | 24 | 100.13 | 299 W | 0.335 | 4.17 |
  | 28 | 110.50 | 299 W | 0.370 | 3.95 |
  | 32 | 122.35 | 300 W | **0.408** | 3.82 |
  **(a) 功耗墙自 12 单元起**,并非全程恒定:4 单元 184 W、8 单元 280 W,
  跨配额功耗极差 **116 W**。故此前「功耗与配额无关」应修正为
  「**12 单元及以上与配额无关**」。切分到 8 单元以下确实省电,但见 (c)。
  **(b) 干扰的量级得到确认**:同一 4 单元配额,含同进程采样时测得
  **12.38 TFLOPS / 299 W**,跨进程采样测得 **22.76 TFLOPS / 184 W**——
  吞吐差 **1.84 倍**,功耗读数差 115 W。这与 kernel trace 的独立结论一致
  (真实比值 6.3× 而非 9.88×)。
  **(c) 两个方向相反的效率排序,均为真,须分别使用**:
  **每单元吞吐**随配额**单调下降**(4 单元 5.69 → 32 单元 3.82 TFLOPS/单元),
  即从「每个 CU 的产出」看,低配额更划算;
  **每瓦吞吐**随配额**单调上升**(0.124 → 0.408),即从「每焦耳的产出」看,
  高配额更划算,32 单元最优,是 4 单元的 **3.3 倍**。
  **调度器据此的取舍**:若瓶颈是 CU(多租户排队),按每单元吞吐决策,
  切分损失小;若瓶颈是功耗或电费,按每瓦吞吐决策,应尽量让单租户用满。
  两者不可混用一个「效率」指标——**这正是此前反复出错的根源**。
...
- 2026-08-06 / scope-is-the-32gb-class-not-four-models（**方向决定**）：
  ...
  **对既有硬门的影响**:此前 `flux-does-not-fit-on-r9700` 等条目曾以
  ...
  不能再以模型装不下为由。
- 2026-08-06 / toy-bench-quota-model-is-the-wrong-form（Gate C 前置）：
  **现有 toy bench 的 quota→speed 模型形式错误,且错误方向会让调度器过度保守。**
  toy bench 用幂律 `speed = sm_share ** 1.08`(指数 > 1,即假设分区本身带来
  超线性损失)。用 Gate B 实测的两张 quota 表检验:
  | 配额 | 实测 SDXL | 实测 CogVideoX-2b | 幂律假设 | Amdahl 拟合 |
  |---|---|---|---|---|
  | 4 | 0.198 | 0.177 | **0.106** | 0.190 |
  | 8 | 0.377 | 0.347 | 0.224 | 0.354 |
  | 16 | 0.665 | 0.657 | 0.473 | 0.621 |
  | 24 | 0.860 | 0.859 | 0.733 | 0.831 |
  | 32 | 1.000 | 1.000 | 1.000 | 1.000 |
  **幂律 MAPE:SDXL 22.0%、CogVideoX-2b 18.0%;Amdahl(serial + parallel/q)
  MAPE:2.0% / 3.2%。** 4 单元处幂律低估达 **1.87 倍**。
  **两种形式的物理含义不同,实测支持后者**:幂律假设「分区本身有超线性损失」,
  Amdahl 假设「存在不随配额缩减的串行部分」。实测的 serial 占比
  (SDXL 37.3%、CogVideoX-2b 25.4%)直接支持后者。
  **错误方向及其后果**:幂律**系统性低估低配额下的性能**,即**高估切分的代价**。
  调度器若据此决策会过度保守、不敢切分——而 Gate B 实测恰恰显示
  4 租户仍保 65% 总吞吐。用错模型会把一个可行的多租户策略排除掉。
  **一个附带的有用性质**:两个模型的**归一化速度曲线高度接近**
  (逐点 0.198/0.177 … 0.940/0.940),尽管其 serial 绝对占比相差 12 个百分点。
  即 quota→speed 的**形状**可跨模型复用,只有 serial/parallel 的绝对值需逐模型
...
- 2026-08-06 / externality-table-symmetric-splits-dominate：**pairwise
  externality table 已产出(4 个配对,SDXL 512²,各 180 s 共同窗口,
  重叠均 >99%)**,并给出三条对调度器直接可用的结论。
  | 配对 | 单元 | solo p50 | co-run p50 | externality | 总吞吐(相对各自 solo) |
  |---|---|---|---|---|---|
  | 对称 | 16+16 | 1.798 / 1.802 | 2.199 / 2.232 | **+22.3% / +23.9%** | **1.625** |
  | 非对称 | 8+24 | 2.949 / 1.444 | 4.408 / 3.282 | +49.5% / **+127.3%** | 1.109 |
  | 非对称 | 24+8 | 1.443 / 2.948 | 3.290 / 4.499 | **+128.0%** / +52.6% | 1.094 |
  | 非对称 | 4+28 | 5.528 / 1.333 | 7.226 / 3.900 | +30.7% / **+192.6%** | 1.107 |
  **(1) 镜像配对完全对称,故外部性取决于配额大小而非物理位置**:
  8+24 与 24+8 互换后,24 单元侧为 +127.3% / +128.0%,8 单元侧为
  +49.5% / +52.6%。这**同时独立验证了 Gate A 的「32 个可掩码单元行为一致」**——
  若某些物理单元更劣,镜像配对就会不对称。
  **(2) 对称切分的总吞吐是非对称的 1.47 倍**(1.625 vs 1.094–1.109),
  且三个非对称配对的总吞吐几乎相同,**与不对称的程度无关**——
  只要非对称就掉到约 1.1。**调度器应优先均分配额**;若必须非对称,
  代价是一次性的、不随比例恶化的。
  **(3) 大配额侧受损远重于小配额侧**,且随配额上升:
  16 单元 +23%、24 单元 +128%、28 单元 **+193%**。与
  `power-wall-starts-at-12-units` 一致:12 单元以上已贴 300 W 功耗墙,
  co-run 时两侧共享同一功耗预算,大配额侧必须让出更多。
  **该不公平来自功耗预算而非 CU 竞争,因而无法用调整 CU 配额来补偿**——
  这是 deadline borrowing 与 resource debt 设计必须纳入的约束。
  **(4) externality 跨度 +22.3% 到 +192.6%,达 8.6 倍**,
...
- 2026-08-06 / partitioning-beats-a-full-die-by-8.8-percent（**Gate C 主结果**）：
  **空间分区的系统吞吐超过「满卡独占」**,在实测标定的仿真中
  **利用率 1.0881(独占为 1.0000),makespan 缩短 8.1%,而平均延迟仅增 2.1%**。
  **基线不是稻草人**:`exclusive_fcfs` 给每个请求**全部 32 个单元**,
  是无空间分区的 runtime 所能做到的最好情形,单请求延迟也是理论最优;
  另加 `oracle_shortest_remaining`(按剩余步数最短优先)作为独占调度的
  延迟下界,二者利用率均为 1.0000。**分区胜出不是因为基线排序差。**
  **机制清楚且由实测支撑**:SDXL 在满卡下有 **39.1%** 的 step 时间是
  不随配额缩减的 serial 部分(CogVideoX-2b 为 27.6%)。独占时每个请求
  **串行地各付一次** serial;分区时两个租户**同时**付各自的 serial,
  故单位时间完成的「满卡等效工作」超过 1.0。
  **利用率的定义须精确**:分子为「已完成 step 数 × 该模型满卡单步耗时」,
  即**满卡等效工作量**,而非墙钟服务时间或 quota-seconds——后两者都会为
  「占住 die」而非「完成工作」记功,而分区策略占住 die 的程度与独占完全相同。
  **过程中修正了两处会削弱本主张的缺陷**:
  (a) **时间推进与实际工作脱钩**:原实现按固定 quantum 推进,而满卡单步
  0.152 s、quantum 0.25 s,每轮空转 39%,独占利用率被报成 0.61;
  更糟的是 16+16 分区单步 0.299 s > quantum,`int(quantum//step)` 恒为 0,
  **两个分区策略完成 0 个请求**。现改为按本轮实际耗时推进,且**每轮至少
  执行一个完整 step**(step 是原子的,不可 checkpoint 于中途)。
  (b) **用拟合替代了可查的实测**:8 个配额点全部实测过,却用 Amdahl 拟合值
  (16 单元 0.622 对实测 0.665)。该残差**并非中性**——它低估低配额性能,
  正好**低估本工作所主张的效应**,把分区增益从 **8.8% 压到 1.6%**。
  现改为「已测配额查表、未测配额才用拟合外推」,并提供 `is_measured()`
  以区分二者。**教训:一个恰好削弱自身主张的「保守」模型,不是安全的默认。**
- 2026-08-06 / single-scalar-counterexample（Gate C 条文）：**「单一标量无法同时
  精确表达进度公平与非线性多资源成本」已由实测数据证成**,且反例不是构造的,
  就是那张 externality 表本身。
  一种虚拟服务货币要担两个角色:**计费**(租户为所获容量付多少)与**成本**
  (这些容量实际买到了多少进度)。若二者可由同一标量担任,则「持有 k 倍的 die」
  必须换来「k 倍的进度」。实测(SDXL,同为 32 单元、只改切分方式):
  | 切分 | 配额比 | 进度比 | 背离(配额比/进度比) |
  |---|---|---|---|
  | 16+16 | 1.00 | 1.00 | **1.000** |
  | 8+24 | 0.33 | 0.38 | 0.889 |
  | 4+28 | 0.14 | 0.31 | **0.460** |
  **最锋利的表述**:在 4+28 下,**小租户手中的一个单元,其产出是大租户手中
  同一个单元的 2.17 倍**。故按「单元·秒」计费,等于对两者按同一价格
  出售两种不同的东西。
  **失效的根源是「成本依赖于对方而非自己」**:同样 8 个单元,配 24 单元时
  externality 为 1.495,而单独运行时为 1.0。任何**仅为自身份额之函数**的
  标量——wall time、quota-seconds、FGE progress、dominant-resource time
  皆是——在原理上就无法表达这一项。
  **该结论的范围须严格限定,不得夸大**:
  (a) **对称切分下单一标量是够用的**(背离恰为 1.000),故结论不是
  「标量无用」;
  (b) **quota-seconds 作为「计费」仍然正确**,且本工作的利用率主张
  (+8.8%)正建立其上——被证否的只是「以同一个数充当成本」。
  这与 plan 的原始表述一致:是「无法**同时**精确表达」二者,而非任一。
- 2026-08-06 / predictor-error-safe-degradation（Gate C 条文）：
  **「predictor error ±5% / ±10% / ±20% 下能够安全降级」已验证,且「安全」
  被给出可检验的定义**:*任何误差水平下,使用预测的策略都不得劣于完全不用
  预测的同一调度器*。错误的预测可以让你失去收益,但不得让你付出超过收益的代价。
  实测(91% 负载、36 请求、deadline slack 4.0,每档 5 个 predictor seed):
  | 预测误差 | deadline_aware 超时(均值/最坏) | static_even(不用预测) |
  |---|---|---|
  | 0% | 11.0 / 11 | 13.0 |
  | 5% | 10.8 / 12 | 13.0 |
  | 10% | 10.4 / 12 | 13.0 |
  | 20% | **11.0 / 13** | 13.0 |
  **即使 20% 误差,最坏情形(13)也只退回到不用预测的基线,不会更差。**
  **两处设计要点,否则该结论会是空洞的**:
  (a) **策略必须只读预测值,不得读真值**——`Predictor` 与执行成本严格分离,
  执行始终用真值。若策略偷看真值,任何调度器都会显得完美鲁棒;
  (b) **场景必须对策略敏感**——过载 trace 无论如何都全超时、轻载 trace
  无论如何都不超时,二者都会给出「完美鲁棒」的假象。故显式断言
  「无误差时 deadline_aware 优于 static_even 且后者超时数 > 0」,
  该断言失败即说明场景本身无判别力。
  **另有一个策略设计上的实测教训**:最初的 `deadline_aware` 在 slack < 0 时
  独占整卡「救急」,结果**比不救更差**(28 对 24 超时)——因为独占放弃了
  8.8% 的分区吞吐,却去救一个已经救不回的请求。改为**仅当独占能改变结果时
  才独占**(分区会超时、独占不会超时的那个区间)后,超时从 13 降到 11。
  **「救急」若不检验能否救回,就是在用系统吞吐换一个必然失败的请求。**
  同时验证 Gate C 的「可行 deadline trace 中不存在可避免 miss」:
  slack 8.0 / 91% 负载下 **0 miss**,且显式断言 horizon 结束时仍在运行的
  请求其 deadline 均在 horizon 之后——否则「0 miss」可能只是 horizon
  结束得早,而非调度成功。
