...
>
> **探测策略的关键前提尚未验证（不得省略）**：探测之所以有价值，依赖
> "**重组配对即重新抽取态**"。真机上已验证的只是"**重启进程**即重新抽取"
> （固定参数 10 次各自独立）。而 plan 设计的 runtime 形态是**单进程、单
> CUDA context、每模型一个 stream**，其中"重组"是改 stream 的 CU mask，
> 并非重启进程。因此：
>
> - 双稳态在**单进程双 stream** 形态下**是否存在**——未测；
> - 若存在，**改 stream mask 是否重新抽取**——未测。
>
> 二者任一为否，探测策略即不可实现，Gate C 的 +13.5% 需重新评估。已知的
> 相关证据只有 transition 探针（运行中改 mask 无瞬态代价，MAPE 0.23%），
> 但那是**单租户**，不涉及配对态。**该实验为当前最高优先级。**
>
