...
> **2026-08-06 新增限定（最终版，前两版已撤回）：co-run 测量是双稳态，
> 分区收益的正负取决于落入哪个态。**
>
> 此处先后写过两版限定并均已撤回：先是"分区收益随分辨率翻转"（归因于工作
> 集），后是"受功耗上限与热状态支配"（归因于热）。**两者都是把单次测量当
> 证据的产物**——真实情况是 co-run 存在双稳态，而被比较的两组各自恰好落在
> 不同态上。
>
> 同一配置（SDXL 768×768、16+16 units）在受控热起点（41–47°C）重复 5 次：
>
> | run | 起始 | 峰值 | 峰值功耗 | co-run/step | per-step externality |
> | --- | --- | --- | --- | --- | --- |
...
>
> 两个态之间没有中间值；态内一致到约 1%（低态逐样本 cv 0.2–0.9%）。热
> 遥测**不能**区分（五次均峰值 89–90°C、309–351 W、同一时钟范围），CU
> mask 也不能（requested / readback / popcount / multiProcessorCount 五次
> 完全相同）。高态的 1.77× 接近完全串行的 2.0×，指向 kernel 并发退化而非
> 竞争加剧；**何者决定态目前未知**。
>
> **对 claim 的后果（直述）**：用两个态各自的 externality 重跑 Gate C 的
> 匹配租户场景——
>
> - 低态（5 次中 2 次）：分区 **+20.8%**
> - 高态（5 次中 3 次）：分区 **−17.8%**
>
> 即**结果取决于卡处于哪个态，而多数观测落在分区吃亏的那个态**。表中的
> `(16,16) = 1.234` 是低态值——是双峰之一，且是少数峰，不是中心值。
>
> **不受影响的部分**：solo 测量五次一致（1.957–1.992 s），故成本表、调度
> 逻辑、记账、lag 界与冻结均不受影响；Gate C 的七条判定是"给定成本模型下
> 调度器是否正确"，仍然成立。受质疑的是 externality 条目，以及经由它的
> **利用率 claim 本身**。
>
> **claim 状态：open（未撤回，亦未确立）**。低态真实且可复现；若态可控，
> claim 加一个条件后成立。**查明何者选择了态，是当前阻塞性的首要问题**，
> 优先于任何进一步的调度器工作。
>
