...
- 2026-08-05 / the-low-quota-anomaly-was-measurement-interference：**「4 单元
  ...
  **低配额下每单元效率更高**,与「4 单元异常低效」的记录相反。成因合理:
  ...
  且不受功耗限制。
  **对既有记录的影响**:`partitioning-costs-aggregate-throughput` 的多租户
  数据同样含该采样线程(采样在驱动进程,对各租户影响均等),故其**绝对值
  偏低约 8%**(1×32 测得 123.76 TFLOPS,干净测量为 134.0),但**相对结论
  不变**——切分损失 35%/66% 的阶梯形状与拐点位置不受等比例偏差影响。
