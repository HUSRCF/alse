...
> **2026-08-06 新增限定：分区收益只在实测的工作点成立，不可外推。**
> 四组 co-run 实测（`experiments/aggregates/pairing_tolerance_20260806.json`）：
>
> | 工作点对 | 步长比 | 配对 | 轮转 | 配对收益 | 规则会配对? |
> | --- | --- | --- | --- | --- | --- |
> | 768 vs 768 | 1.000 | 198.1 ms | 231.0 ms | **+16.6%** | 是 ✓ |
> | 1024 vs 1024 | 1.000 | 531.1 ms | 401.0 ms | **−24.5%** | 是 ✗ |
> | 1024 vs 1152 | 1.333 | 665.5 ms | 460.5 ms | −30.8% | 是 ✗ |
> | 1024 vs 1280 | 1.693 | 796.1 ms | 533.3 ms | −33.0% | 否 ✓ |
>
> 前两行是对照：**步长比同为 1.000，结论相反**。决定配对是否划算的不是
> 冻结规则所读的步长比，而是工作点本身——per-step externality 从 768 的
> +22.4%/+24.5% 涨到 1024 的 +71.2%/+81.1%（两侧都是 16 units）。
>
> 因此：(a) 仿真内部自洽（成本表与 externality 均测于 768，其算得的
> +18.9% 与 768 实测的 +16.6% 接近）；(b) **该结果不能外推到其他工作点**，
> 不是关于 SDXL 的普遍结论，更不是关于空间分区的普遍结论；(c) 冻结的
> 1.6 步长比阈值**没有实测支撑**，四组中只在一组给出正确判断。正确的判据
> 应比较 makespan 并需要逐工作点的 externality 预测，而表目前只有 768 一
> 个点——在补足测量前不改规则，以免用隐性错误替换显性错误。
