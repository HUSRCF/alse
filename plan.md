...
> **2026-08-06 新增限定：co-run 测量受功耗上限与热状态支配，必须冷启动。**
>
> 先前此处写有一张"分区收益随分辨率翻转"的表并据此断言 1.6 阈值判据错误。
> **该表已撤回**——它是热态假象。同一配置（768×768、16+16）三次独立测量：
>
> | 运行 | solo/step | co-run/step | externality |
> | --- | --- | --- | --- |
> | 原测（当天首个 co-run，冷） | 157.5 ms | 192.7 ms | **+22.4%** |
> | 重测（连跑数小时后，热） | 157.1 ms | 282.8 ms | **+80.1%** |
> | 冷却至 45°C 后重测 | 159.0 ms | 192.7 ms | **+21.2%** |
>
> solo 三次一致（1% 内），两次冷态 co-run 精确一致，只有热态偏离 47%。
> 运行中热监控：junction 达 **90°C**，sclk 由 2567 降至 2350 MHz，功耗
> **299–301 W** 对 **300 W** 上限。**单租户不触上限，双租户触顶**——被我
> 归因于"工作集变大"的惩罚，实际是功耗上限同时压制两个租户。
>
> 因此：(a) simulator 所用的 `(16,16) = 1.234` 反而**得到加强**（两次冷
> 态测量一致），Gate C 的 +18.9% 成立；(b) "该结果不能外推到其他工作点"
> 这一断言**被撤回**——它基于被污染的对照，现状是**未知**而非已证伪；
> (c) 此前所有热态 co-run（784/800/816/832/896/1024 及两组失配对）作为
> 原始证据保留并标注，但不可用作工作点对比。
>
> **由此暴露一个设计未建模的变量**：两个租户各 16 units 会触及 300 W 上
> 限，而单租户 32 units 不会——算力可用而功率预算不可用。这比它所替代的
> "工作集"解释更重要。
>
> **方法约束（即刻生效）**：co-run 仅在受控热起点下可比；每次用于对比的
> co-run 必须记录起始 junction 温度与功耗/时钟轨迹，不同热状态的测量不得
> 相互比较。
