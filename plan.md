...
> **2026-08-06 新增限定：分区收益成立，但条件是租户启动必须错开 2–5 秒。**
>
> 此处先后写过三版限定并均已撤回：先归因于"工作集阈值"，再归因于"功耗
> 上限"，再记为"双稳态且不可控"。**前两版都是把单次测量当证据的产物**；
> 第三版描述正确但结论过早——态是可控的。
>
> co-run 的 per-step externality 呈**双峰且互不重叠**，由两个租户进程的
> **启动间隔**选择：
>
> | 启动错开 | 次数 | 态 | externality | 逐样本 cv |
> | --- | --- | --- | --- | --- |
> | 0 s | 3 | 高 | +73.7~77.8% | 1.6–2.4% |
> | **2 s** | **2** | **低** | **+24.3, +24.8%** | **0.21–0.25%** |
> | **3 s** | **2** | **低** | **+24.8, +24.8%** | **0.23–0.25%** |
> | **5 s** | **5** | **低** | **+22.8~24.9%** | **0.19–0.25%** |
> | 8 s | 2 | 高 | +75.3, +79.8% | 2.2–2.7% |
> | 10 s | 2 | 高 | +75.1, +75.3% | 2.4–2.9% |
> | 15 s | 3 | 高 | +72.9~83.4% | 1.9–2.6% |
>
> 窗口内 9/9 落低态，窗口外 11/11 落高态。低态不仅更快，逐样本 cv 还低
> 一个数量级。低态 18 个侧测量均值 **1.2362**，高态 20 个侧测量均值
> **1.7866**，两组**完全不重叠**。
>
> **对 claim 的后果**：低态下分区 **+20.8%**，高态下 **−17.8%**。表中
> `(16,16) = 1.2362` 是低态值，现由 18 次测量支撑（原 1.234 由 2 次得出，
> 二者相差 0.18%——旧值本就落在低态，属幸运而非稳健）。
>
> 因此 **Gate C 的 +18.9% 成立，但附带一个必须写进运行时的条件**：租户
> 启动不得同时、也不得相隔 ≥8 秒。态在第一步之前就已确定，且**不由**温度、
> 功耗、任何时钟、GPU 利用率或 CU mask 区分（以上均已测量）。
>
> **一个尚未验证的假设**：该效应可能属于"冷启动 co-run 测量"而非稳态服务
> ——本 harness 每次 co-run 都重新加载 SDXL，而常驻模型的 runtime 不会
> 每次租户切换都重建 context。可用"单进程双 stream + 不相交 mask"（即
> Gate A 的形态）验证，**此处未验证，故仅记为假设**。
>
> **方法约束**：单次 co-run 测量不构成证据；任何 externality 结论必须基于
> 重复测量的分布，且必须记录启动错开量。
